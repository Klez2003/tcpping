...
{
	if (useColor)
		print_c::FormattedPrint(PRINT_COLOR_BLUE, i18n_c::GetString(error));
	else
		print_c::FormattedPrint(0, i18n_c::GetString(error));  // Changed NULL to 0
...
int printConnectInfo(host_c host)
{
	...
	if (useColor)
		print_c::FormattedPrint(PRINT_COLOR_CYAN, info);
	else
		print_c::FormattedPrint(0, info);  // Changed NULL to 0
...
int printSuccessfulConnection(host_c host, double time)
{
	...
	if (useColor)
		print_c::FormattedPrint(PRINT_COLOR_RED, data);
	else
		print_c::FormattedPrint(0, data);  // Changed NULL to 0
...
int printStats()
{
	...
	if (useColor)
		print_c::FormattedPrint(PRINT_COLOR_GREEN, str);
	else
		print_c::FormattedPrint(0, str);  // Changed NULL to 0
...
{
	if (useColor)
		print_c::FormattedPrint(PRINT_COLOR_MAGENTA, i18n_c::GetString(error));
	else
		print_c::FormattedPrint(0, i18n_c::GetString(error));  // Changed NULL to 0
